...
export default class MapPage extends React.Component {// create an externally visisble (export) component
  ...
  render() {//what to display
    console.log('rendering')
                ...
                <Modal key = {Math.floor(Math.random() * 1000000)} animationType={'fade'} visible= {this.state.reportModalVisible}  transparent={true} >
                    <View style={styles.overlay}>
                      <View style={styles.container}>
                        <Text style={styles.title2}>REPORT DETAILS</Text>
                        <Text style={styles.title}>IS THERE LIGHT?: {doc.status}</Text>
                        <Text style={styles.title}>REPORT TIME: {doc.time}</Text>
                        <Text style={styles.title}>REPORT ADDRESS: {doc.address}</Text>
        ...
        <Modal key = {Math.floor(Math.random() * 1000000)} animationType={'fade'} visible= {this.state.notFoundModalVisible}  transparent={true} >
          <View style={styles.overlay}>
            <View style={styles.container}>
              ...
              <Text style={styles.title}>The address you typed in was not found!</Text>
              <Text style={styles.title}>Cross-check the address and try again </Text>
              <Text style={styles.title}>You were not charged for this {'\n'}</Text>
              <Text style={styles.plainText}>HINT: Google search the location to get an exact address</Text>
              <TouchableOpacity style={styles.closeButton} onPress={this.onCloseNotFoundModal}>
                <Text style={styles.buttonText}>CLOSE</Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <TouchableOpacity style={styles.loadButton} onPress={this.onClickLoad} disabled= {this.state.disableLoad} >
        ...
        <Text style={styles.settingsTitle}>Set Time interval:</Text>
        <Picker
        ...
        <View style= {styles.zoomContainer}>
          ...
          <Text style={styles.settingsTitle}>Zoom Level:</Text>
          <View style= {styles.markerZoomContainer}>
            <BaseButton style={styles.zoomButton} onPress={this.onZoomIn}>
              <Text style={styles.zoomButtonText}>+</Text>
            </BaseButton>
            <BaseButton style={styles.zoomButton} onPress={this.onZoomOut}>
              <Text style={styles.zoomButtonText}>-</Text>
            </BaseButton>
          </View>

          <Text style={styles.settingsTitle}>Indicator Size:</Text> 
          <View style= {styles.markerZoomContainer}>
            <BaseButton style={styles.zoomButton} onPress={this.onMarkerZoomIn}>
              <Text style={styles.zoomButtonText}>+</Text>
            </BaseButton>
            <BaseButton style={styles.zoomButton} onPress={this.onMarkerZoomOut}>
              <Text style={styles.zoomButtonText}>-</Text>
            </BaseButton>
          </View>

          <Text style={styles.settingsTitle}>Clear Zoom:</Text> 
          <View style= {styles.markerZoomContainer}>
            <BaseButton style={styles.clearZoomButton} onPress={this.onClearAllZoom}>
...
<View style={styles.container3}>
  <Text style={styles.title2}>WHAT'S NEARBY</Text>
  <TextInput placeholder= "search items and places here"  
    style={styles.location2} 
    onChangeText={(text) => this.setState({inputAddr: text})} 
    value={this.state.inputAddr} 
    onSubmitEditing = {() => this.onSubmitAddress(this.state.inputAddr)}/> 
    <TouchableOpacity style={styles.button} onPress={this.onClickLoad} disabled= {this.state.disableLoad} >
        <Text style={styles.buttonText}>CLOSE</Text>
    </TouchableOpacity>
</View>
...
<Search ref="search_box"
            placeholder= {this.state.inputAddr} 
            onSearch= {() => this.onSubmitAddress(this.state.inputAddr)}
            style={styles.location2} 
            onChangeText={(text) => this.setState({inputAddr: text})} 
            value={this.state.inputAddr} 
            onSubmitEditing = {() => this.onSubmitAddress(this.state.inputAddr)}/> 
...
<View style={styles.container3}>
  <TouchableOpacity style={styles.button} onPress={this.onOpenShops}>
        <Text style={styles.buttonText}> NEARBY PRODUCTS</Text>
    </TouchableOpacity>
</View>
